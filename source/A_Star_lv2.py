...
def AStar_Lv2_heuristic_1(matrix, start, end, bonus_points):
...
def AStar_Lv2_heuristic_2(matrix, start, end, bonus_points):
    frontier = PriorityQueue()
    bonus_points_frontiers = dict()
    bonus_points_list = [(i[0], i[1]) for i in bonus_points]
    for bp in bonus_points_list:
    ...
    startNode = Node(state=start, parent=None,  g=0, h=h_func2(start, end))
    frontier.put(startNode)

    if bonus_points_frontiers:
        for bp, bp_frontier in bonus_points_frontiers.items():
            bp_frontier.put(Node(state=start, parent=None,  g=0, h=h_func2(start, bp)))
        ...
        for neighBor in getNeighbors(current.state, matrix):
            if neighBor not in visited:
                ...
                if bonus_points_frontiers:
                    for bp, bp_frontier in bonus_points_frontiers.items():
                        # print(bp)
                        bp_frontier.put(Node(state=neighBor, parent=current, g=current.g+1, h=h_func2(neighBor, bp)))
